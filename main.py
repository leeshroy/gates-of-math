...
def play():
    ...
    while not dead:
        ...
        for gate in gates:
            ...
            if gate['rect'].y > height:  # Check if the gate has moved off-screen
                gate['rect'].y = random.randint(-100, -10)  # Reset the gate to above the top of the screen
                problemSet = createMathProblems.createProblemSet()
                newAnswers = createMathProblems.createAnswerSet(problemSet)
                gate['effect'] = newAnswers[gates.index(gate)]  # Update the gate's effect based on new problems
                gate['active'] = True  # Make sure the gate is active for collision detection
                gate['collided'] = False  # Reset collision status
